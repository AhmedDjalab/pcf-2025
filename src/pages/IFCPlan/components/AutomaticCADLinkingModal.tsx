...
interface AutomaticCADLinkingModalProps {
  viewer: Autodesk.Viewing.GuiViewer3D;
  setActivities: React.Dispatch<React.SetStateAction<ActivityModel[]>>;
  activities: ActivityModel[];
}

const AutomaticCADLinkingModal = ({
  viewer,
  activities,
  setActivities,
}: AutomaticCADLinkingModalProps) => {
  const [showModal, setShowModal] = useState(false);
  const [activityId, setActivityId] = useState("ActivityUID");
  const [synchroCode, setSynchroCode] = useState("UID-");
  const [isLinking, setIsLinking] = useState(false);
  const [linkSuccess, setLinkSuccess] = useState(false);
  const [error, setError] = useState("");
  const getUIDBlocksBulk = (
    viewer: Autodesk.Viewing.GuiViewer3D
    ...
    return new Promise((resolve, reject) => {
      ...
      model.getBulkProperties(
        ...
        (results) => {
          ...
          results
            .filter((result) => result.name && result.name.startsWith("UID-"))
            .forEach((result) => {
              // Remove "UID-" prefix to get just the identifier part
              const uidKey = result.name
                .replace("UID-", "")
                .replace(/\s*\[.*?\]/, "");

              // Add dbId to the array for this UID key
              if (synchroCodeMap[uidKey]) {
                synchroCodeMap[uidKey].push(result.dbId);
              } else {
                synchroCodeMap[uidKey] = [result.dbId];
              }
            });

          console.log(
            `🎉 Found ${Object.keys(synchroCodeMap).length} unique UID codes`
          );
          console.log("📊 Synchro Code Map:", synchroCodeMap);

          resolve(synchroCodeMap);
        },
        reject
      );
    });
  ...
  const handleLink = async () => {
    ...
    const synchroCodeMap = await getUIDBlocksBulk(viewer);
          ...
          <div className="w-full max-w-md overflow-hidden transition-all duration-300 transform scale-95 bg-white shadow-2xl rounded-xl animate-in fade-in-90 zoom-in-90">
            ...
            <div className="p-5 text-white bg-gradient-to-r from-purple-500 to-purple-600">
              ...
              <p className="mt-1 text-sm text-purple-100">
                Connect activities to Autocade elements using UID
              </p>
            </div>
            ...
            <div className="p-5">
              ...
              <div className="grid grid-cols-1 gap-4 mb-5 md:grid-cols-2">
                <div>
                  ...
                  <input
                    type="text"
                    defaultValue="activityUid"
                    value={activityId}
                    disabled={true}
                    onChange={(e) => setActivityId(e.target.value)}
                    placeholder="activityUid"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Enter autocade code field"
                    className="w-full p-3 transition-colors border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
              </div>
...
export default AutomaticCADLinkingModal;
function getBlocksGroupedByUID(
  viewer: Autodesk.Viewing.GuiViewer3D,
  arg1: string
) {
